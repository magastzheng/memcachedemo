...
namespace BeIT.MemCached {
	class Example {
	    ...
	    {
            MemcachedClient cache = MemcachedClient.GetInstance("MyConfigFileCache");
	        cache.Set("name", "Magast Zheng", DateTime.Now.AddMinutes(20));

	        var result = cache.Get("name");
            Console.WriteLine("Get the data of key name as: " + result);
	        cache.Delete("name");
            Console.WriteLine("Delete the cache of key name");
	        result = cache.Get("name");
            Console.WriteLine("Get again the data of key name as: " + (result ?? "null"));

            Console.WriteLine("Check the memcache");
